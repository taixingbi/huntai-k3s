...
Endpoints: `POST /v1/rerank`, `GET /health`, `GET /ready`, `GET /version`, `GET /metrics`, `GET /docs`. In-cluster: `http://layer-gateway-reranker:8000`; LAN: NodePort `30182` — see [port.md](port.md) for host IPs. vLLM backends (`:8002`): [deploy-vllm-reranker.md](deploy-vllm-reranker.md). Prometheus job `vllm-reranker` scrapes `vllm-rerank-gpu-node-*` with `workload=reranker` ([prometheus-configmap.yaml](../manifests/observability/prometheus-configmap.yaml)). Grafana: `grafana-import/dashboard/reranker.json`. Smokes: [test-calls.md](test-calls.md).
